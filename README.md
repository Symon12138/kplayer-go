...
### 🔀 与原版 KPlayer 的差异（本仓库调整版）

本仓库在原版基础上将推流核心替换为 **FFmpeg 子进程引擎**，并新增了 Web 播控台与管理层。与原版的能力对照：

| 能力 | 原版（libkplayer） | 本仓库（FFmpeg 引擎） |
|---|---|---|
| 多资源循环/顺序/随机推流 | ✅ | ✅（另支持目录逐集连续播放） |
| 多路输出 + 断线重连 | ✅ | ✅ |
| 视频混音/外挂音轨 | ✅（多输入混合） | ✅（视频+外挂音频+字幕按时间戳合并） |
| 编码参数 | 全局 encode + avg_quality/fill_strategy | 线路级分辨率/码率/帧率/声道/采样率/编码器/硬件加速/滤镜 |
| 预生成缓存（cache_on） | ✅ | ❌ 实时转码，无缓存机制 |
| 播放模型 queue | ✅ | ❌（order/loop/random/random-loop） |
| WASM 插件生态 | ✅（可加载第三方 .wasm 插件） | ⚠️ 内置 7 类效果（水印/字幕/音频/跑马灯/画面调整/转码预设）以 ffmpeg 原生滤镜等效实现并注入推流命令行；**不能加载官方 WASM 插件** |
| Web 管理控制台（18 页全功能） | ❌ | ✅（媒体库/节目单/定时任务/推流任务/效果/告警/Webhook/审计/用户权限/统计等） |

> 原生 `core/`（libkplayer cgo 封装）保留在仓库中，`native/` 提供 API 兼容的 stub 构建方案；
> 生产部署推荐使用 FFmpeg 引擎路径（默认）。
