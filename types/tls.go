...
func init() {
	// The TLS material is injected at build time via -ldflags. With an
	// empty configuration (plain development builds) skip decoding instead
	// of failing: there is no certificate to decrypt and no client token
	// to load, and every TLS consumer checks for nil/empty data.
	if TlsRootCert != "" {
