	...
	for _, item := range resources {
		...
		if !kptypes.FileExists(resourceFilePath) {
			if err := InitResourceFile(item["type"], item["name"], resourceFilePath); err != nil {
				// A missing optional resource (e.g. the default font in an
				// offline or development build without an API host) must not
				// block startup: the plugin still works, only the resource is
				// absent. Downgrade to a warning instead of failing.
				logField.WithField("error", err).Warn("resource initialization failed; continuing without it")
			} else {
